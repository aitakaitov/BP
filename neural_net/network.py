...
def create_model(train_dir: str, test_dir: str, preprocessor, model_path=None, tb_logdir=None,
    ...
    validation.validate_model(model, preprocessor, "../validation_datasets/validation_dataset_dev_fp",
                   "../validation_datasets/validation_dataset_dev_fp_annotated")

    print("Validating the model against true labels of FP+TN dataset")
    validation.validate_model(model, preprocessor, "../validation_datasets/validation_dataset_dev_extended",
    ...
    validation.validate_model(model, preprocessor, "../validation_datasets/validation_dataset_control",
...
def get_model(preprocessor):
    ...
    if not sequential:
        ...
        model.add(layers.Conv1D(filters=32, kernel_size=4, kernel_initializer=tensorflow.keras.initializers.he_normal()))
        model.add(layers.MaxPooling1D(pool_size=3))
        model.add(layers.Activation(tensorflow.keras.activations.relu))
        model.add(layers.Conv1D(filters=32, kernel_size=8, kernel_initializer=tensorflow.keras.initializers.he_normal()))              # aktivace po poolingu relu
